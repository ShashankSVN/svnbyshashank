package collection;
import java.util.HashSet;
import java.util.Iterator;
import java.util.Set;


public class HashSetWithPuppy {
	public static void main(String[] args) {
		
     Set<Puppy> temp=new HashSet<Puppy>();
     Puppy p11=new Puppy("KKKK",100);
	 Puppy p12=new Puppy("PPPP",120);
	 temp.add(p11);
	 temp.add(p12);
	 //default capacity of hashset 16	
	 Set<Puppy> set=new HashSet<Puppy>();
	 //it can only hold object of type Puppy? yes
	 Puppy p1=new Puppy("MMMM",10);
	 Puppy p2=new Puppy("AAAA",12);
	 Puppy p3=new Puppy("CCCC",5);
	 Puppy p4=new Puppy("BBBB",4);
	 Puppy p5=new Puppy("AAAA",12);
	 //adding elements inside the set
	 set.add(p1);
	 set.add(p2);
	 set.add(p3);
	 set.add(p4);
	 set.add(p5);
	 set.addAll(temp);
	 //displaying all the elements
	 //p2 and p5 contains different hashCode , that's why we are able 
	 //to add duplicate element in set and violating the it's contract
	 for(Puppy p : set){
		 System.out.println(p.toString());//p.toString();
	 }
	 
	 //Accessing the elements of Set using Iterator interface
	 Iterator<Puppy> it=set.iterator();
	 
	 while(it.hasNext()){
		 Puppy puppy=it.next();
		 System.out.println("Puppy detail = "+puppy);
	 }
	 
	 
	 
	}

}
